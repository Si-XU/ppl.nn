...
void HorizTuringIMMAImpgemm::DeleteAttrParam(void*& param) {
    delete (CudaHorizConvParam*)param;
    return;
}

void HorizTuringIMMAImpgemm::GetAttrParam(void*& param) const {
    if (param == nullptr)
        param = new CudaHorizConvParam();
    *(CudaHorizConvParam*)param = attr_param_;
    return;
}

bool HorizTuringIMMAImpgemm::IsSupported(const ir::Node* node, const OptKernelOptions& options,
                                    dataformat_t input_format) const {
    uint32_t group = (reinterpret_cast<CudaHorizConvParam*>(options.param))->param.group;
...
double HorizTuringIMMAImpgemm::ExcuteTimer(const ir::Node* node, OptKernelOptions& options) {
    this->attr_param_ = *(reinterpret_cast<CudaHorizConvParam*>(options.param));
    attr_param_.extra_param.algo_info.algo_type = "HorizTuringIMMAImpgemm";
    ...
    temp_attr_param.param = attr_param_.param;
    temp_attr_param.extra_param.bias_term = attr_param_.extra_param.bias_term_list[0];
    ...
    auto group = ((CudaHorizConvParam*)options.param)->param.group;
    ...
    auto timer = PPLCUDAConvolutionJitSelectKernelInt8(
        ...
        (int4*)output_buffer.addr, (int4*)bias_buffer.addr, nullptr, attr_param_.extra_param.algo_info,
...
RetCode HorizTuringIMMAImpgemm::ModifyParam(ir::Node* node, OptKernelOptions& options) {
    this->attr_param_ = *(reinterpret_cast<CudaHorizConvParam*>(options.param));
    ...
    for (uint32_t item = 0; item < attr_param_.extra_param.size; ++item) {
        auto weight_index_list = attr_param_.extra_param.weight_index_list[item];
        const TensorShape& shape_in1 = *options.tensors->find(node->GetInput(weight_index_list))->second->GetShape();
        const TensorShape& shape_out = *options.tensors->find(node->GetOutput(item))->second->GetShape();

        temp_attr_param.extra_param.bias_term = attr_param_.extra_param.bias_term_list[item];
        ConvertToForwardConvParam(shape_in0, shape_in1, shape_out, temp_attr_param, temp_conv_param);
        ...
        auto channel_per_grp_pad = (channel_per_grp + align_size - 1) / align_size * align_size;
        auto total_size = channel_per_grp_pad * group;
        auto& weight_quant = options.quants->at(node->GetInput(weight_index_list));
        ...
        *options.tensors->find(quant_edge_id)->second->GetShape() = quant_shape;
        ...
        if (weight_iter != data->constants.end() && // is a constant tensor and has not be loaded
            ...
            auto postedge_id = node->GetInput(weight_index_list);
        ...
        reinterpret_cast<CudaHorizConvParam*>(options.param)->extra_param.is_initializer_weight_list[item] =
            weight_iter != data->constants.end();

        if (attr_param_.extra_param.bias_term_list[item] == 0) {
            continue;
        }

        // Split bias format to group padding
        auto bias_edge = topo->GetEdge(node->GetInput(weight_index_list + 1));
        ...
        if (bias_iter != data->constants.end() && // is a constant tensor and has not be loaded
            ...
            auto postedge_id = node->GetInput(weight_index_list + 1);
            const TensorShape& preshape = *options.tensors->find(preedge_id)->second->GetShape();
            const TensorShape& postshape = *options.tensors->find(postedge_id)->second->GetShape();
            auto newshape = postshape;
            newshape.SetDim(0, k_per_grp_pad * temp_conv_param.num_grp);
...
void HorizTuringIMMAImpgemm::ReshapeOnEdges(const ir::Node* node, std::map<edgeid_t, std::unique_ptr<TensorImpl>>* tensors,
