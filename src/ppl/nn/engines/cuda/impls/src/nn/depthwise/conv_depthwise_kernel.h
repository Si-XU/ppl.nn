...
template<int TILE_H, int TILE_W, int SRC_TILE_H, int SRC_TILE_W, int KERNEL_H, int KERNEL_W, int STRIRDE_H, int STRIRDE_W>
__global__ void ppl_cuda_depthwise_int8_nhwc(
    const int8_t* input,
    const int8_t* filter,
    const float* bias,
    DivModFast padc_fast,
...
#pragma unroll
    ...
    if ((in_w >= 0) && (in_w < in_width)) {
        flt_data[0] = int_filter[flt_off + 3*c_stride];
        flt_data[1] = int_filter[flt_off + 4*c_stride];
        flt_data[2] = int_filter[flt_off + 5*c_stride]; 
...
//2
    ...
    if( (in_w+2>=0) && (in_w+2<in_width) ){
        flt_data[0] = int_filter[flt_off + 6*c_stride];
        flt_data[1] = int_filter[flt_off + 7*c_stride];
    ...
    fuse_process_float<4, 1>(a + 0, h_idx, w_idx, c_idx, out_height, out_width, channels, paddingc, base_offset, fuse_params);
...
#pragma unroll
    ...
    int outData;
    int* dout = (int*)output;
    outData = (C[ 0]<<0)+(C[ 1]<<8)+(C[ 2]<<16)+(C[ 3]<<24);
    if(h_idx*4+0<out_height) dout[out_idx] = outData; out_idx += out_width * (paddingc >> 2);
    outData = (C[ 4]<<0)+(C[ 5]<<8)+(C[ 6]<<16)+(C[ 7]<<24);
    if(h_idx*4+1<out_height) dout[out_idx] = outData; out_idx += out_width * (paddingc >> 2);
    outData = (C[ 8]<<0)+(C[ 9]<<8)+(C[10]<<16)+(C[11]<<24);
    if(h_idx*4+2<out_height) dout[out_idx] = outData; out_idx += out_width * (paddingc >> 2);
    outData = (C[12]<<0)+(C[13]<<8)+(C[14]<<16)+(C[15]<<24);
