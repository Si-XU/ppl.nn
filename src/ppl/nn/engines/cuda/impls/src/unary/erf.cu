...
#include "cudakernel/unary/erf.h"
#include <cuda_fp16.h>

#if __CUDA_ARCH__ >= 600 && __CUDACC_VER_MAJOR__ >= 9
template <typename T>
__device__ __inline__ T ppl_scalar_erf(const T &in_val)
{
    return erf(in_val);
}

template <>
__device__ __inline__ half ppl_scalar_erf<half>(const half &in_val)
{
    return __float2half(erf(__half2float(in_val)));
}

template <>
__device__ __inline__ int8_t ppl_scalar_erf<int8_t>(const int8_t &in_val)
{
...
template <typename T>
__global__ void ppl_cukernel_erf_ndarray(const uint64_t num_elems,
                                         const T *input,
                                         T *output)
{
#if __CUDA_ARCH__ >= 600 && __CUDACC_VER_MAJOR__ >= 9
    ...
    if (index >= num_elems)
        return;
    output[index] = ppl_scalar_erf<T>(input[index]);
#endif
}

template <typename T>
__global__ void ppl_cukernel_erf_nhwc(const uint64_t num_elems,
                                      int channels,
                                      int pad_channels,
                                      int chw,
                                      int hw,
    ...
    void *output)
{
    ...
    if (output_shape->GetDataFormat() == ppl::common::DATAFORMAT_NDARRAY) {
        ...
        if (output_shape->GetDataType() == ppl::common::DATATYPE_FLOAT32) {
            ppl_cukernel_erf_ndarray<float><<<grid_size, block_size, 0, stream>>>(num_elems,
                                                                                  (const float *)input,
                                                                                  (float *)output);
        } else if (output_shape->GetDataType() == ppl::common::DATATYPE_FLOAT16) {
            ppl_cukernel_erf_ndarray<half><<<grid_size, block_size, 0, stream>>>(num_elems,
                                                                                 (const half *)input,
                                                                                 (half *)output);

        } else if (output_shape->GetDataType() == ppl::common::DATATYPE_INT8) {
            ppl_cukernel_erf_ndarray<int8_t><<<grid_size, block_size, 0, stream>>>(num_elems,
                                                                                 (const int8_t *)input,
                                                                                 (int8_t *)output);
        } else {
