...
double TuringIMMAImpgemm::ExcuteTimer(const ir::Node* node, OptKernelOptions& options) {
    ...
    if (algo_info != options.algos->end()) {
        ...
        attr_param_.extra_param.algo_info.algo_name = "nvSwzlSm75Int8Conv_imma8816_nhwc_fn_b256x64_w64x64_k64_buf2_spk1_spf1";
    ...
    PPLCUDAPredictInt8ConvKernel(device_id, shape_in0.GetDataType(), attr_param_.extra_param.algo_info, temp_conv_param);
