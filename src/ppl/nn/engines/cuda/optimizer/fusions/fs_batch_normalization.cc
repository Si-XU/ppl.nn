...
const bool BatchNormalizationFusion::CanFuse(ir::Node* nextnode, const OptKernelOptions& options, uint32_t flag) {
...
const RetCode BatchNormalizationFusion::FuseBatchNormalizationWithNextNode(ir::Node* node, ir::Node* nextnode, const OptKernelOptions& options) {
...
const RetCode BatchNormalizationFusion::FuseNode(ir::Node* node, bool reliable, const OptKernelOptions& options) {
    ...
    for (uint32_t i = 0; i < 1 && node->GetOutputCount() == 1; ++i) {
        ...
        if (CanFuse(nextnode, options, i)) {
            ...
            param->extra_param.has_activation = 1;
            options.info->kernels.erase(nextnode_id);
            FuseBatchNormalizationWithNextNode(node, nextnode, options);
        }
    }
    return RC_SUCCESS;
}
