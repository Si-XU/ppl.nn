...
void HorizTuringHMMAImpgemm::DeleteAttrParam(void*& param) {
    delete (CudaHorizConvParam*)param;
    return;
}

void HorizTuringHMMAImpgemm::GetAttrParam(void*& param) const {
    if (param == nullptr)
...
bool HorizTuringHMMAImpgemm::IsSupported(const ir::Node* node, const OptKernelOptions& options,
...
double HorizTuringHMMAImpgemm::ExcuteTimer(const ir::Node* node, OptKernelOptions& options) {
    this->attr_param_ = *(reinterpret_cast<CudaHorizConvParam*>(options.param));
    attr_param_.extra_param.algo_info.algo_type = "HorizTuringHMMAImpgemm";
...
RetCode HorizTuringHMMAImpgemm::ModifyParam(ir::Node* node, OptKernelOptions& options) {
    this->attr_param_ = *(reinterpret_cast<CudaHorizConvParam*>(options.param));
...
void HorizTuringHMMAImpgemm::ReshapeOnEdges(const ir::Node* node, std::map<edgeid_t, std::unique_ptr<TensorImpl>>* tensors,
