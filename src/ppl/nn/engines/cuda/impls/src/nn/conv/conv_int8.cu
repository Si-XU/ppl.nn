    ...
    uint64_t workspace)
{
    ...
    int index = 0;
    std::vector<const char *> compile_params;
    double elapsed = AlgoForwardTimeInt8(device_id, stream, knames, sources, index, compile_params, device_id, true, type, d_input, d_flt, d_output, bias, d_temp_buf, params, conv_param, quant_param, fuse_param, workspace);
