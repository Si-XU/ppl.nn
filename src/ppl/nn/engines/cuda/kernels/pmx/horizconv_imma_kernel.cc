...
ppl::common::RetCode HorizConvImmaKernel::BeforeExecute(KernelExecContext* ctx) {
...
ppl::common::RetCode HorizConvImmaKernel::DoExecute(KernelExecContext* ctx) {
    ...
    for (uint32_t item = 0; item < param_->extra_param.size; item++) {
        ...
        temp_quant_param[item].in_scale = input_scale;
        temp_quant_param[item].out_scale = 1 / output_scale;
        temp_quant_param[item].d_flt_scale = d_weight_scale;
        if (temp_fuse_param[item].has_elt) {
            auto tps = param_->extra_param.fuse_info_list[item].types;
            auto ret = std::find(tps.begin(), tps.end(), "Add");
            if (ret == tps.end())
                LOG(ERROR) << "fuse_info types error: no add op";
            int id = ret - tps.begin();
            auto elt_index = param_->extra_param.fuse_info_list[item].input_ind[id];
            auto elt = ctx->GetInput<TensorImpl>(elt_index);
            auto elt_quant = GetCommonParam()->cuda_tensor_info->at(elt->GetEdge()->GetId());
            temp_quant_param[item].pre_scale = elt_quant.scale[0];
        }
        if (param_->extra_param.fuse_info_list[item].channel_offset >= 0) {
            temp_quant_param[item].out_scale =
                1 / GetCommonParam()->cuda_tensor_info->at(param_->extra_param.fuse_info_list[item].concat_edge_id).scale[0];
        }
    }
...
// #endif
    LOG(DEBUG) << "Excute IMMA horizconv with kernel id:" << param_->extra_param.algo_info.kid;
